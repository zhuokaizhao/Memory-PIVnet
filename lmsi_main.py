...
def main():
    ...
    if mode == 'generate_pt_dataset':
        if verbose:
            print(f'\nmode: {mode}')
            print(f'\nIn {mode} mode, input_dir is the directory that contains image/label data generated by the Julia part of the project.')
            print('\noutput_dir in this mode is the directory where pt dataset is saved to')
        # assign to local parameters
        data_dir = args.input_dir[0]
        ...
        # The directory should contain train, val and test
    ...
    # training with customized manager (newest)
        ...
        loss_path = os.path.join(figs_dir, f'{network_model}_loss_{starting_epoch+num_epoch}.png')
        plt.savefig(loss_path)
        print(f'\nLoss graph has been saved to {loss_path}')

        # save model as a checkpoint so further training could be resumed
        model_path = os.path.join(model_dir, f'{network_model}_{time_span}_{starting_epoch+num_epoch}.pt')
